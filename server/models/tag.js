...
module.exports = function (mongoModels) {
    ...
    function getTagEditInfo(userName, fileName, resultCallback) {
        mongoModels.util.findLatestFileVersion(fileName, function (lastVersion, lastDocumentId) {
            ...
            mongoModels.commitInfo.findById(lastDocumentId, function (error, doc) {
    ...
    function applyTagEditInfo(fileName, tagNames, resultCallback) {
        mongoModels.util.findLatestFileVersion(fileName, function (lastVersion, lastDocumentId) {
            ...
            mongoModels.commitInfo.findOneAndUpdate({ _id: lastDocumentId }, { tag_names: tagNames }, function (error, docs) {
                if (error) {
                    throw error;
                }
                resultCallback({});
    ...
    function addTags(commitDocumentId, addTagNames) {
        return (function () {
            var d = $.Deferred();
            mongoModels.commitInfo.findById(commitDocumentId, function (error, doc) {
                if (error) {
                    d.reject(error);
                    return;
                }
            ...
            mongoModels.commitInfo.findOneAndUpdate({ _id: commitDocumentId }, { tag_names: newTagNames }, function (error, docs) {
                if (error) {
                    d.reject(error);
                    return;
                }
