...
module.exports = function (mongoModels) {
    ...
    function find(findProvision, resultCallback) {
        mongoModels.latestCommitId.find({}, function (error, docs) {
            ...
            mongoModels.commitInfo.find(findQuery, function (error, commitInfoDocs) {
                ...
                mongoModels.userLastViewCommitVersion.findOne({ user_name: findProvision.userName }, function (error, lastViewDoc) {
    ...
    function _updateLastViewFileVersion(userName, fileName, fileVersion) {
        mongoModels.userLastViewCommitVersion.findOne({ user_name: userName }, function (error, foundDoc) {
            if (error) {
                throw error;
            }
            if (foundDoc) {
                ...
                // 既存のドキュメントを削除
                mongoModels.userLastViewCommitVersion.findByIdAndRemove(foundDoc._id, function (error) {
                    if (error) {
                        throw error;
                    }
                    // 新しくドキュメントを保存
    ...
    function usersView(fileName) {
        return (function () {
            ...
            userNames.forEach(function (userName) {
                mongoModels.userLastViewCommitVersion.findOne({ user_name: userName }, function (error, doc) {
                    if (error) {
                        d.reject(error);
                        return;
                    }
    ...
    function _updateLatestCommitList(commitInfoDocId, previousCommitInfoDocId, resultCallback) {
        if (previousCommitInfoDocId) {
            ...
            mongoModels.latestCommitId.findOne({ 'commit_doc_id': previousCommitInfoDocId }).remove(function (error) {
                if (error) {
                    throw error;
                }
            });
