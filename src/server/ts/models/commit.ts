 ...
 */
export function checkSafety(fileNames, userName): any {
 ...
 */
export function find(findProvision, resultCallback): any {
        ...
        .findOne({ user_name: findProvision.userName }, function(error, lastViewDoc) {
          var result = _.map(commitInfoDocs, function(doc) {
            ...
            return {
              ...
              commit_time: (<any>new Date(doc.commit_time)).toFormat('YYYY/MM/DD HH24:MI'),
...
function createCommitInfoFindQuery(commitDocIds, findProvision) {
    var findQuery: any = { _id: { $in: commitDocIds } };
 ...
 */
export function download(user, documentId, resultCallback): any {
 ...
 * @param resultCallback 結果を返すコールバック
 */
export function downloadWithVersion(documentId, resultCallback): any {
 ...
 */
export function usersView(fileName): any {
  return (function() {
    var d = new $.Deferred();
