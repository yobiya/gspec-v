...
export function setup(selecterObjects) {
  ...
  selecters.$dialog.setup = function(historyInfoArray, isDiffSupport) {
    ...
    historyInfoArray.forEach(function(historyInfo) {
      ...
      var $tableRow = $('<tr>');
      Util.appendTableRowCell($tableRow,
                                      $checkBox,
  ...
  selecters.$diffDialog.setup = function(oldVersion, newVersion, oldDiffHtml, newDiffHtml, diffInfo) {
    ...
    var $leftDoc = $((<any>$this.find('#left_iframe')[0]).contentDocument.documentElement);
    var $rightDoc = $((<any>$this.find('#right_iframe')[0]).contentDocument.documentElement);
...
function convertViewHtml(htmlText, deleteLineNumbers, changeLineNumbers, addLineNumbers) {
  ...
  // ヘッダは無視する
  (function() {
    var bodyLineIndex = 1 + _.findIndex(lines, function(line: any) {
